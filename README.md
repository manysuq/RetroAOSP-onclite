...
> **LineageOS 22.2 (Android 15)**. Честные патчи поверх реальных исходников:
> каждая цель патча сверена с деревом `lineage-22.2`, ресурсы переопределяются
...
| Геометрия | Углы 2dp у диалогов, кнопок, уведомлений, плиток QS | overlay: `dialog_corner_radius`, `control_corner_material`, `notification_corner_radius`, … |
...
База — LineageOS 22.2 (Android 15): последняя ветка LineageOS с деревьями Redmi 7. Android 16 для onclite не существует в открытых исходниках (порт с 4PDA закрытый); при появлении ветки lineage-23 у дерева устройства CI это заметит.
...
еженедельно проверяет, что цели патчей не «уехали» в апстриме `lineage-22.2`, а сборка — локальная.
...
./build_local.sh sync           # repo init (lineage-22.2) + repo sync
...
.github/workflows/validate.yml  # линтер + слежение за апстримом lineage-22.2
```
...
3. **Цели существуют в реальности**: все строки сверены с живыми исходниками
   `lineage-22.2` (июль 2026), CI перепроверяет их еженедельно.
